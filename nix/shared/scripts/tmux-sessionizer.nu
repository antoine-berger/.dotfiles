...
# credits: https://github.com/amtoine/tmux-sessionizer
...
def switch-to-or-create-session [session: record<name: string, path: path>]: nothing -> nothing {
    ...
    # switch-client will fail if not in a tmux session
    if ($env.TMUX? | is-empty) {
        log debug $"attaching to ($session.name)"
